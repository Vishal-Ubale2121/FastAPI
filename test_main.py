...
@pytest.mark.parametrize("number,expected", [(4, "Number is even"), (3, "Number is odd")])
def test_check_even(number, expected):
    response = client.get(f"/checkEven/{number}")
    assert response.status_code == 200
    assert response.json()["output"] == expected

@pytest.mark.parametrize("number,expected", [(2, True), (4, False), (17, True), (1, False), (0, False)])
def test_check_prime(number, expected):
    response = client.get(f"/checkPrime/{number}")
    assert response.status_code == 200
    assert response.json()["output"] == expected

@pytest.mark.parametrize("text,upper,lower", [("Hello", True, True), ("HELLO", True, False), ("hello", False, True)])
...
@pytest.mark.parametrize("text,expected", [("racecar", True), ("hello", False), ("A man, a plan, a canal, Panama", True)])
...
@pytest.mark.parametrize("email,expected", [("test@example.com", True), ("invalid_email", False)])
...
def test_find_vowels():
    ...
    assert response.status_code == 200
    data = response.json()["output"]
    assert "A" in ''.join(data["vowels"]).upper() or "E" in ''.join(data["vowels"]).upper()
